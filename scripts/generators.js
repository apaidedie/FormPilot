...
function setGeoapifyApiKey(key) {
  ...
  formPilotLog('Geoapify API Key 已' + (geoapifyApiKey ? '设置' : '清除'));
}
...
async function fetchRealAddressFromApi(lat, lon) {
  ...
  try {
    ...
    if (!response.ok) {
      formPilotLog('Geoapify API 请求失败:', response.status);
      return null;
    }
    ...
    formPilotLog('Geoapify API 调用失败:', e);
  }
...
async function fetchAddressFromOSM(lat, lon) {
  ...
  try {
    const response = await fetch(url, {
      headers: {
        'User-Agent': 'FormPilot-Extension/1.8.0'
      }
    });

    if (!response.ok) {
      formPilotLog('OSM Nominatim API 请求失败:', response.status);
      return null;
    }
    ...
    formPilotLog('OSM Nominatim API 调用失败:', e);
  }
...
async function fetchRealAddressSmart(lat, lon) {
  ...
  if (osmResult) {
    formPilotLog('使用 OpenStreetMap 地址');
    return osmResult;
  }
...
const NAME_DATABASE = {
  ...
  // 意大利语名
  it: {
    firstNames: ['Francesco', 'Alessandro', 'Lorenzo', 'Matteo', 'Leonardo', 'Sofia', 'Giulia', 'Aurora', 'Ginevra', 'Alice',
      'Tommaso', 'Riccardo', 'Andrea', 'Federico', 'Gabriele', 'Martina', 'Chiara', 'Beatrice', 'Elena', 'Valentina'],
    lastNames: ['Rossi', 'Russo', 'Ferrari', 'Esposito', 'Bianchi', 'Romano', 'Colombo', 'Ricci', 'Marino', 'Greco',
      'Bruno', 'Gallo', 'Conti', 'De Luca', 'Mancini', 'Costa', 'Giordano', 'Rizzo', 'Lombardi', 'Moretti']
  },
  // 巴西葡萄牙语名
  pt: {
    firstNames: ['João', 'Pedro', 'Lucas', 'Gabriel', 'Miguel', 'Maria', 'Ana', 'Julia', 'Beatriz', 'Luiza',
      'Rafael', 'Gustavo', 'Felipe', 'Bruno', 'Thiago', 'Mariana', 'Camila', 'Larissa', 'Fernanda', 'Isabela'],
    lastNames: ['Silva', 'Santos', 'Oliveira', 'Souza', 'Rodrigues', 'Ferreira', 'Alves', 'Pereira', 'Lima', 'Gomes',
      'Costa', 'Ribeiro', 'Martins', 'Carvalho', 'Almeida', 'Lopes', 'Soares', 'Fernandes', 'Vieira', 'Barbosa']
  },
  // 印度常见罗马化名
  hi: {
    firstNames: ['Aarav', 'Vihaan', 'Arjun', 'Rohan', 'Aditya', 'Ananya', 'Diya', 'Priya', 'Kavya', 'Isha',
      'Rahul', 'Karan', 'Nikhil', 'Vikram', 'Amit', 'Neha', 'Pooja', 'Aditi', 'Riya', 'Meera'],
    lastNames: ['Sharma', 'Verma', 'Patel', 'Gupta', 'Singh', 'Kumar', 'Shah', 'Mehta', 'Reddy', 'Iyer',
      'Nair', 'Joshi', 'Rao', 'Chopra', 'Malhotra', 'Agarwal', 'Desai', 'Kapoor', 'Bose', 'Menon']
  },
  // 荷兰语名
  nl: {
    firstNames: ['Daan', 'Bram', 'Luuk', 'Sem', 'Lars', 'Sophie', 'Emma', 'Julia', 'Tess', 'Lotte',
      'Noah', 'Milan', 'Finn', 'Thomas', 'Jesse', 'Sara', 'Eva', 'Nora', 'Fleur', 'Anna'],
...
const MEIGUODIZHI_API_URL = 'https://www.meiguodizhi.com/api/v1/dz';
...
const US_STATE_PATHS = {
  ...
  louisiana: { slug: 'lousiana', name: 'Louisiana', code: 'LA' },
...
const US_STATE_ALIASES = Object.values(US_STATE_PATHS).reduce((aliases, state) => {
  aliases[state.name.toLowerCase()] = state;
  aliases[state.code.toLowerCase()] = state;
  aliases[state.slug.toLowerCase()] = state;
  return aliases;
}, {});

const US_CITY_ALIASES = {
...
const CITY_STATE_MAP = {
  ...
  'Netherlands': [
    { city: 'Amsterdam', state: 'North Holland', zip: '10' },
    { city: 'Rotterdam', state: 'South Holland', zip: '30' },
    { city: 'The Hague', state: 'South Holland', zip: '25' },
    { city: 'Utrecht', state: 'Utrecht', zip: '35' },
    { city: 'Eindhoven', state: 'North Brabant', zip: '56' },
    { city: 'Groningen', state: 'Groningen', zip: '97' }
  ]
};
...
function generateZipCode(country) {
  ...
  if (country === 'United States') {
    ...
    return zipPrefix + randomDigits(4 - zipPrefix.length) + ' ' + randomChoice(letters.split('')) + randomChoice(letters.split(''));
  }
...
function normalizeLocationInput(location) {
  return (location || '')
    ...
    .replace(/,$/, '')
    .replace(/\s+state$/i, '')
    .replace(/^(city|state)\s+/i, '');
}

function getUsStateByLocation(location) {
  const normalized = normalizeLocationInput(location).toLowerCase().replace(/-/g, ' ');
  return US_STATE_ALIASES[normalized] || null;
}

function buildMeiguodizhiRequest(location) {
  const target = normalizeLocationInput(location);
  const cityAlias = US_CITY_ALIASES[target.toLowerCase()];
  if (cityAlias) {
    return {
      city: cityAlias,
      path: '/',
  ...
  const state = getUsStateByLocation(target);

  if (state) {
    return {
      city: '',
      path: `/usa-address/${state.slug}`,
      method: 'address',
      target: state.name
    };
  }
  ...
  return {
    city: target,
    path: '/',
    method: target ? 'refresh' : 'address',
    target: target || 'United States'
  };
}

function parseUsBirthday(value, fallbackSettings = {}) {
  if (!value) return generateBirthday(fallbackSettings.minAge || 18, fallbackSettings.maxAge || 55);

  const match = String(value).trim().match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/);
  if (!match) return generateBirthday(fallbackSettings.minAge || 18, fallbackSettings.maxAge || 55);
...
function splitFullName(fullName, country) {
  ...
  return {
    firstName: parts[0] || generateFirstName(country),
    lastName: generateLastName(country)
  };
}

function mapMeiguodizhiAddress(rawAddress, settings = {}) {
  const country = 'United States';
  const name = splitFullName(rawAddress.Full_Name, country);
  const username = rawAddress.Username || generateUsername(name.firstName, name.lastName);

  return {
    firstName: name.firstName,
    lastName: name.lastName,
    gender: String(rawAddress.Gender || '').toLowerCase() === 'female' ? 'female' : 'male',
    birthday: parseUsBirthday(rawAddress.Birthday, settings),
    username: username,
    email: rawAddress.Temporary_mail || generateEmail(username),
    password: rawAddress.Password || generatePassword(),
    phone: rawAddress.Telephone || generatePhone(country),
    address: String(rawAddress.Address || generateAddress(country)).replace(/\s+/g, ' ').trim(),
    city: rawAddress.City || generateCity(country),
    state: rawAddress.State_Full || rawAddress.State || generateState(country),
    zipCode: rawAddress.Zip_Code || generateZipCode(country),
    country: country,
    sensitive: {
      creditCardType: rawAddress.Credit_Card_Type || '',
      creditCardNumber: rawAddress.Credit_Card_Number || '',
      creditCardExpires: rawAddress.Expires || '',
      creditCardCvv: rawAddress.CVV2 || '',
      ssn: rawAddress.Social_Security_Number || '',
      monthlySalary: rawAddress.Monthly_Salary || '',
      employmentStatus: rawAddress.Employment_Status || '',
...
async function fetchMeiguodizhiProfile(location, settings = {}) {
  const request = buildMeiguodizhiRequest(location);
  const response = await fetch(MEIGUODIZHI_API_URL, {
    method: 'POST',
    headers: {
  ...
  if (!response.ok) {
    throw new Error(`meiguodizhi 请求失败: ${response.status}`);
  }

  const result = await response.json();
  if (result.status !== 'ok' || !result.address) {
    throw new Error('meiguodizhi 未返回可用地址');
  }

  return {
    ...mapMeiguodizhiAddress(result.address, settings),
    requestedLocation: request.target
  };
}
...
if (typeof window !== 'undefined') {
  ...
  function generateAllInfoWithSettings(ipData, settings = {}) {
    ...
    if (country === 'Japan' && window.japanGenerators) {
      ...
      return {
        ...
        city: japanAddr.city,
        state: japanAddr.prefecture,
        zipCode: japanAddr.zipCodeFormatted,
        country: country,
        prefectureJp: japanAddr.prefecture,
        cityJp: japanAddr.city,
        chomeJp: japanAddr.chome,
        buildingJp: japanAddr.building,
        phoneJp: japanPhone,
  ...
  window.generators = {
    ...
    fetchMeiguodizhiProfile: fetchMeiguodizhiProfile,
      ...
      if (coords) {
        try {
          ...
          if (realAddr && realAddr.address) {
            formPilotLog('获取真实地址成功:', realAddr.address, '来源:', realAddr.source);
            return realAddr;
          }
        } catch (e) {
          formPilotLog('地址 API 失败，使用本地生成');
        }
      }
